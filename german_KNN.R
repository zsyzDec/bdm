...
# gdata<-cbind(dat$V2,dat$V5,dat$V13,df.onehot,as.factor(dat$V21))
...
for(k in c(1:10)){
    ...
    test       <- gdata[testind,-21]
    test.lable <- gdata[testind,'V21']
    train      <- gdata[-testind,-21]
    train.lable      <- gdata[-testind,21]
    ...
    nn <- knn(train = train, test = test, cl = train.lable, k=5)
