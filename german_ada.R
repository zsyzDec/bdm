...
for(k in c(1:10)){
    ...
    r          <- predict(ada,  newdata=train[,-21])
    acc.ada.train<-acc.ada.train+bacc(r$class, train[,21], 1)
    
    start <- end +1
}
print(paste( "acc ada test:",acc.ada/10))
print( (acc.ada[1]+acc.ada[2])/2)
print(paste( "acc ada train:",acc.ada.train/10))