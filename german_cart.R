...
for(k in c(1:10)){
    ...
    r         <- predict(ct.pruned, train, type='class')
    acc.cart.train  <- bacc(r,train.lable,1)+acc.cart.train
    
    start <- end +1
}
print(paste( "acc cart test:",acc.cart/10))
print( (acc.cart[1]+acc.cart[2])/2)
print(paste( "acc cart train:",acc.cart.train/10))